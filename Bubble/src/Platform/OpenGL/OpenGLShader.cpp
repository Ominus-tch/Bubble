...
namespace Bubble {
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			CreateProgram();
			BG_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			...
			{
				BG_CORE_ERROR("Could not read from file '{0}'", filepath);
			}
		}
		else
		{
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			BG_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			BG_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
