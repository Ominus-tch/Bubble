...
namespace Bubble {
	...
	namespace Utils {
		...
		inline const char* ScriptFieldTypeToString(ScriptFieldType fieldType)
		{
			...
			BG_CORE_ASSERT(false, "Unknown ScriptFieldType");
			return "None";
		}
		...
		inline ScriptFieldType ScriptFieldTypeFromString(std::string_view fieldType)
		{
			...
			if (fieldType == "Shader") return ScriptFieldType::Shader;
		...
		static std::string ExtractNamespaceClass(const std::string& filename)
		{
			...
			std::regex classRegex(R"(public\s+class\s+(\w+)\s*:\s*Entity)");
			...
			while (std::getline(file, line)) {
				...
				// If both namespace and class are found, no need to continue
			...
			if (!namespaceName.empty() && !className.empty()) {
				...
				return ""; // Return an empty string if either is not found
			}
		}
