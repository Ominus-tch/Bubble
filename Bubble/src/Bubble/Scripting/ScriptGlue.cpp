...
#define BG_ADD_INTERNAL_CALL(Name) mono_add_internal_call("Bubble.InternalCalls::" #Name, Name)
	...
	{
		BG_CORE_WARN("Value: {0}", *parameter);
	...
	static bool Entity_HasComponent(UUID entityID, MonoReflectionType* componentType)
	{
		...
		BG_CORE_ASSERT(scene);
		Entity entity = scene->GetEntityByUUID(entityID);
		BG_CORE_ASSERT(entity);

		MonoType* managedType = mono_reflection_type_get_type(componentType);
		BG_CORE_ASSERT(s_EntityHasComponentFuncs.find(managedType) != s_EntityHasComponentFuncs.end());
	...
	static MonoString* TextComponent_GetText(UUID entityID)
	{
		...
		Entity entity = scene->GetEntityByUUID(entityID);
		BG_CORE_ASSERT(entity);
		BG_CORE_ASSERT(entity.HasComponent<TextComponent>());
		...
		([]()
			{
				...
				std::string managedTypename = fmt::format("Bubble.{}", structName);

				MonoType* managedType = mono_reflection_type_from_name(managedTypename.data(), ScriptEngine::GetCoreAssemblyImage());
				if (!managedType)
				{
					BG_CORE_ERROR("Could not find component type {}", managedTypename);
					return;
				}
